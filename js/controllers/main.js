...
// =========================================================================
    ...
    .constant('USER', {
        "USER_ROLES": ["Student", "Faculty Member", "Other"],
    ...
    .controller('RegisterCtrl', function RegisterCtrl($scope, fireFactory, $rootScope, USER) {
        ...
        $rootScope.model.user.index.participatedPollCount = 0;
        $rootScope.model.user.index.createdPollCount = 0;
    })
    ...
    .controller('PollCtrl', function PollCtrl($scope, fireFactory, $rootScope, $state, pollQuestion, pollQuestionOption, currentPoll, $filter, $sce, ngTableParams, $location, growlService) {
        ...
        $rootScope.$watch('searchTerm', function () {
            ...
            if ($rootScope.searchTerm.length > 0) {
                ...
                angular.forEach($scope.searchListPolls, function (value, key) {
                    ...
                    if (tempTitle.search(tempSearchTerm) > -1) {

                        var matchesForTitle = searchResult.filter(function (datum) {
                            return datum.key === key;
                        });

                        if (!matchesForTitle.length) {

                            if (arrayObjectIndexOf(searchResult, key, 'pollID') > -1) {
        ...
        $scope.userPollParticipateInteraction = function (pollId) {
            ...
            if (!$rootScope.currentUserReference.currentUserData.index) {
                $rootScope.currentUserReference.currentUserData.index = {};
            }

            if (!$rootScope.currentUserReference.currentUserData.index.participatedPollCount) {
                $rootScope.currentUserReference.currentUserData.index.participatedPollCount = 0;
            }

            $rootScope.currentUserReference.currentUserData.index.participatedPollCount++;
        ...
        $scope.userPollCreateInteraction = function (pollId) {
            ...
            if (!$rootScope.currentUserReference.currentUserData.index.createdPollCount) {
                $rootScope.currentUserReference.currentUserData.index.createdPollCount = 0;
            }

            $rootScope.currentUserReference.currentUserData.index.createdPollCount++;
            ...
            $scope.specificPoll.$loaded().then(function (loadedData) {
                ...
                angular.forEach(loadedData.question, function (qValue, qKey) {
                    ...
                    pollQuestion.add({
                        ...
                        option: $scope.pollQuestionOptions.slice(),
                        placeHolder: qValue.placeHolder
                    });
